...
# 1. 1행 마다 tuple로 묶어준다. = row
# 2. 1~20행 까지의 tuple을 또 tuple로 묶어준다. = column
# 3. index를 활용해서 인접한 숫자 4개의 조합을 모두 구한 후 가장 큰 값을 찾는다.
...
def max_product_in_grid(grid):
    ...
    # 격자 내 모든 셀에 대해 가능한 4개의 연속된 숫자들의 곱을 계산하며 최대값을 출력
