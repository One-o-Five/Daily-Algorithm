...
둘째 줄에 순열 A의 원소 A(i)가 공백으로 구분되어 주어진다. 모든 A(i)는 1 이상 N 이하의 서로 다른 정수임이 보장된다.
...
def can_clean_sequence(n, a):
    stacks = [[] for _ in range(4)]  # 네 개의 스택 초기화
    
    for num in a:
        placed = False
        for stack in stacks:
            # 현재 스택이 비어 있거나, 스택의 최상단 값보다 작거나 같은 경우
            if not stack or stack[-1] <= num:
                stack.append(num)
                placed = True
                break
        
        # 네 개의 스택 중 어떤 곳에도 배치할 수 없다면 NO
        if not placed:
            return "NO"
